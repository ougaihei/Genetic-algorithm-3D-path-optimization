...
%     population(i).ds=LENGTH/N;
    ...
    [population(i).alpha population(i).theta population(i).x population(i).y population(i).z population(i).fitness population(i).error_tvd population(i).sum population(i).length]=trajGenerate(N,TARGET_ALPHA,TARGET_THETA,TARGET_TVD,LENGTH,ds,lambda,C);
...
while(Gen<=500)
    ...
    offspring2.lambda=offspring2.lambda+offspring1.sigma_lambda*normrnd(0,1);
    offspring2.C=offspring2.C+offspring1.sigma_C*normrnd(0,1);
    %evaluate offspring fitness and trajectory
    [offspring1.alpha offspring1.theta offspring1.x offspring1.y offspring1.z offspring1.fitness offspring1.error_tvd offspring1.sum offspring1.length]=trajGenerate(N,TARGET_ALPHA,TARGET_THETA,TARGET_TVD,LENGTH,ds,offspring1.lambda,offspring1.C);
    [offspring2.alpha offspring2.theta offspring2.x offspring2.y offspring2.z offspring2.fitness offspring1.error_tvd offspring2.sum offspring2.length]=trajGenerate(N,TARGET_ALPHA,TARGET_THETA,TARGET_TVD,LENGTH,ds,offspring2.lambda,offspring2.C);
    ...
    hold on
    text(population(1).x(length(population(1).x)),population(1).y(length(population(1).y)),population(1).z(length(population(1).z)), sprintf('%i', Gen))
    hold on
    end
